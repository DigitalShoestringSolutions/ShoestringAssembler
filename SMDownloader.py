...
with solution_files.joinpath(Path(recipefilename)).open(mode='r') as recipefile:
    ...
    for line in recipefile:
        ...
        # Lines take the form url=tag. Split them up:
        line = line.split("=")
        line[-1] = line[-1].split("\n")[0]  # Remove trailing newline from last item.

        # Associate names
        url = line[0]                       # use everything before the first = as the url
        sm_base_name = url.split("/")[-1]   # everything after the last / in the url, i.e. the repo slug = name
