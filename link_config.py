...
# Creates Unix hard links (non-symbolic) between parts of the UserConfig directory and parts of the Modules directory.
# If the UserConfig contains the subdir MyModule and MyModule is successfully downloaded via recipe.txt, 
# then the contents of UserConfig/MyModule/ will be hard linked into Modules/MyModule/config/.
...
print("## -----------------------------------------------------------------------")
print("Linking UserConfig to Modules...")
print("    UserConfig path:    ", UserConfig)
print("    Modules path:", Modules)

# For each subdirectory of UserConfig:
for config_module_dir in UserConfig.glob('*'):
# config_module_dir is a full absolute path. Extract module name only from config_module_dir.relative_to(UserConfig) below.
    ...
    # configitem is a full absolute path. 
        ...
        dest_path = Modules.joinpath(config_module_dir.relative_to(UserConfig), "config", configitem.relative_to(config_module_dir))
        ...
        # if not a directory, it is a file item that can be linked
            ...
            # Hard link from the file in UserConfig to the config folder in the Module
